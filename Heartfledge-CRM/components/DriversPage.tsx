...
const DriversPage: React.FC<DriversPageProps> = ({ data }) => {
  ...
  const handleUpdateStatus = (driverId: number, newStatus: string) => {
    setDrivers(prev => prev.map(d =>
      d.id === driverId
        ? { ...d, employment_status: newStatus as EmploymentStatus, updated_at: new Date().toISOString() }
        : d
    ));
    // Update selectedDriver if it's the one being modified
    if (selectedDriver && selectedDriver.id === driverId) {
      setSelectedDriver(prev => prev ? { ...prev, employment_status: newStatus as EmploymentStatus, updated_at: new Date().toISOString() } : null);
    }
  };
